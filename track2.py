...
VIDEO_PATH = 'inputs/videos/camera2.mp4'
...
area_far = (200, 300, 800, 450)  # Example coordinates for area_far
...
while cap.isOpened() and frame_index < frames_limit:
    ...
    # Draw the area_near rectangle in red
    cv2.rectangle(frame, (area_near[0], area_near[1]), (area_near[2], area_near[3]), (0, 0, 255), 2)
...
for obj_id, frames in tracking_data.items():
    ...
    # Check if the first occurrence is in area_near and the last in area_far or vice versa
    if (is_in_area(first_center, area_near) and is_in_area(last_center, area_far)) or (is_in_area(first_center, area_far) and is_in_area(last_center, area_near)):
        # Filter the bounding boxes that do not exist in area_near or area_far
        filtered_frames = []

        for frame in frames:
            frame_center = (frame[1][0] + frame[1][2]) // 2, (frame[1][1] + frame[1][3]) // 2

            if is_in_area(frame_center, area_near) or is_in_area(frame_center, area_far) or frame[1][2]> 750:
                continue
